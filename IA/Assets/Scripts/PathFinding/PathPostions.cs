...
public class PathPostions : MonoBehaviour {
	...
	void Start () {
        ...
        List<PathNode> path = dijkstra.getPath(originAndTargetNodes[5].First);
	...
	private void SetNodes(List<PairOfTransform> positions){
		 for (int i = 0; i < positions.Count; i++) {
			 PathNode origin = new PathNode("Node_o_" + i, "Node_o_" + i, positions[i].First.position);
			 PathNode destination = new PathNode("Node_t_" + i, "Node_t_" + i, positions[i].Second.position);
			 originAndTargetNodes.Add(new Pair<PathNode, PathNode>(origin, destination));
		 }
	}
